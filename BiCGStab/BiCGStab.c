...
double alpha(struct CSR_matrix *m, double *r_0_help, double *r_n, double *p_n) {
...
int s(struct CSR_matrix *m, double *r_n, double *p_n, double alpha_n, double*s_n) {
...
double omega(struct CSR_matrix *m, double *s_n) {
...
int BiCGStab(struct CSR_matrix *m, double *b) {
	...
	for (int num = 0; num < 1000; num++) {
		...
		alpha_n = alpha(m, r_0_help, r_n, p_n);

		s(m, r_n, p_n, alpha_n, s_n);
